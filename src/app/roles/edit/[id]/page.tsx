...
export default function EditRolePage() {
      ...
      .then((data) => {
        ...
        if (!hasAccess) {
          toast.error("Akses Ditolak", {
            description: "Anda tidak memiliki wewenang untuk memodifikasi peran.",
          });
          router.replace("/roles");
          return;
        }
        // Fetch detail role & permissions secara paralel
    ...
    Promise.all([
      ...
      .then(([roleData, permsData]) => {
        ...
        if (r.name === "Superadmin") {
          toast.error("Entitas Terlindungi", {
            description: "Peran Superadmin tidak dapat dimodifikasi.",
          });
          router.replace("/roles");
          return;
        }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    fetch(`${CONFIG.API_BASE_URL}/roles/${roleId}`, {
      method: "PUT",
      headers: {
      ...
      .then(() => {
        toast.success("Peran berhasil dimodifikasi!");
        router.push("/roles");
      })
      .catch((err) => {
        toast.error(err.message);
